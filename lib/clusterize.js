...
function clusterize(workerFn, masterFn, reforkOnDeath, workersToCoresRatio, workersCount, workerDeathCallback, env) {
    ...
    if (cluster.isMaster) {        
        ...
        if (typeof(masterFn) === 'function') {
            ...
            var masterResolved = resolveMasterPath(masterFn);
            console.log(masterResolved);
            require(masterResolved);
        }
...
function resolveMasterPath(masterPath) {
    ...
    if (fs.existsSync(module.parent.filename)) {
        var stat = fs.statSync(module.parent.filename);

        if (stat.isFile()) {
            return path.resolve(path.dirname(module.parent.filename), masterPath);

        } else  if (stat.isDirectory()) {

            return path.resolve(module.parent.filename, masterPath);

        } else {
            throw new Error('module parent id could not be used ' + module.parent.filename + ' must be a file or a directory');
        }

    } else {
        throw new Error('could not resolve path to master, module parent id is ' + module.parent.filename);
    }
}
