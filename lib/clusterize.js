...
function resolveMasterPath(masterPath) {
    ...
    if (fs.existsSync(module.parent.id)) {
        var stat = fs.statSync(module.parent.id);

        if (stat.isFile()) {
            return path.resolve(path.dirname(module.parent.id), masterPath);
        } else  if (stat.isDirectory()) {
            return path.resolve(module.parent.id, masterPath);
        } else {
            throw new Error('module parent id could not be used ' + module.parent.id + ' must be a file or a directory');
        }

    } else {
        throw new Error('could not resolve path to master, module parent id is ' + module.parent.id);
    }
}
