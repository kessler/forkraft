...
function on(eventFunction, type, callback, target) {
	function messageHandler(message) {
		if (message.__type && message.__type === type)
			callback(message.__payload);
	}

	if (target) {
		// caller specified an explicit target
		target[eventFunction]('message', messageHandler);
		...
		//master
		for (var id in cluster.workers) {
			cluster.workers[id][eventFunction]('message', messageHandler);
		}

	} else {

		// worker
		_process[eventFunction]('message', messageHandler);
	}
}
